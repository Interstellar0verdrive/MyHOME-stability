...
`);if(a.every(Vi)){e.push(s`<ul>
  ...
  </div>`,pi=n=>{let{i18n:t}=n;return s`<div class="strip pending-bar">
    <span class="count"
  ...
  >`};var hi=u`.row{position:relative;display:flex;align-items:flex-start;gap:8px;padding:8px;border-radius:8px;min-height:48px;-webkit-user-select:none;-webkit-touch-callout:none}.row .divider{position:absolute;left:8px;right:8px;top:-1px;height:1px;background:var(--myhome-divider);opacity:.6;pointer-events:none}.row .insert-line{position:absolute;left:8px;right:8px;top:-2px;height:3px;border-radius:2px;background:var(--myhome-primary-ink);pointer-events:none}.row .pending-outline{position:absolute;inset:0;border:2px dashed var(--myhome-primary-ink);border-radius:8px;pointer-events:none}.row .source-veil{position:absolute;inset:0;background:var(--myhome-background-soft);opacity:.7;border-radius:8px;pointer-events:none}.row .handle{width:48px;height:48px;flex:0 0 48px;border:none;background:transparent;color:var(--myhome-text-soft);cursor:grab;font-size:18px;letter-spacing:2px;border-radius:8px;touch-action:none;-webkit-user-select:none;user-select:none}.row .handle[disabled]{cursor:default;color:var(--myhome-text-off)}@media(max-width:599px){.row .handle{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip-path:inset(50%);white-space:nowrap}.row .handle:focus-visible{position:static;width:48px;height:48px;margin:0;overflow:visible;clip-path:none}}.row .body{flex:1 1 auto;min-width:0}.row .main{display:block;width:100%;text-align:left;border:none;background:transparent;color:inherit;font:inherit;cursor:pointer;padding:2px 0;border-radius:6px}.row .name{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;font-size:14.5px;line-height:1.35}.row .sub{display:block;font-size:12.5px;color:var(--myhome-text-soft);margin-top:2px}.row .chips:empty{display:none}.row .chips{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px;align-items:center}.row .warn{display:block;font-size:12.5px;color:var(--myhome-warning-ink);margin-top:4px}.route{display:inline-flex;align-items:center;gap:2px;font-size:12px;color:var(--myhome-primary-ink);background:var(--myhome-primary-faint);border:1px dashed var(--myhome-primary-ink);border-radius:10px;padding:2px 2px 2px 8px;max-width:100%}.route .text{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.route .withdraw{width:48px;height:48px;margin:-14px -14px -14px 0;display:inline-flex;align-items:center;justify-content:center;border:none;background:transparent;color:inherit;font:inherit;font-size:14px;line-height:1;cursor:pointer;border-radius:24px}.row .note{display:block;font-size:12.5px;color:var(--myhome-info-ink);margin-top:4px}`,rn=(n,t,e)=>{let i=t.height!==null?n.t("panel.overview.cover.travel",{travel:n.number(t.height,0)}):e?n.t("panel.overview.cover.travel_needed"):n.t("panel.overview.cover.travel_unknown");return t.area?`${t.area} · ${i}`:i},on=(n,t)=>t.has_own.length===0?"":L.every(e=>t.has_own.includes(e))?n.t("panel.assign.pending.all_own"):n.t("panel.assign.pending.some_own"),ui=(n,t)=>{let{i18n:e,pending:i}=t,r=!!i&&i.to!==null&&n.height===null,o=i?on(e,n):"",a=`chips-${n.unique_id}`;return s`<div class="row" data-row=${n.unique_id}>
    ...
    >
      ...
      <div class="chips" id=${a}>
        ${Me(e,n.origin,n.profile,t.short)}
        ${i?s`<span class="route">
  ...
  >
    <span class="line"
      ...
      >${n.current===i?s`<span class="tag">${t.t("panel.dialog.option.current")}</span>`:l}</span
    >
  ...
  >
    ...
    <div class="sheet" role="dialog" aria-modal="true" aria-labelledby="review-title" data-focus-root>
      ...
      <div class="body" aria-busy=${n.previewing?"true":"false"}>
        ...
        ${i.map(({change:p,cover:d})=>{let c=e.get(p.cover),m=n.heights[p.cover],h=p.to!==null&&d.height===null,v=h?ie(m):null,_=v!==null&&(n.forced||(m??"")!==""),H=c&&c.problem===null?o.filter(w=>c.values[w]!==void 0&&d.values[w]!==void 0):[];return s`<section class="item">
            <div class="line">
            ...
            ${h?s`<label class="travel">
                  ...
                  ${_?s`<p class="field-error">
                        ${v==="missing_travel"?t.t("panel.review.travel.required"):wi(t,d,v,p)}
                      </p>`:l}`:l}
            ...
            ${(()=>{let w=an(t,d,p,c,n.profiles);return w?s`<p class="note">${w}</p>`:l})()}
          </section>`})}
  ...
  `};var Le=class extends f{constructor(){super();this._trap=new $e;this._returnTo=null;this._returnToRow=null;this._onKey=e=>{if(e.key==="Escape"){if(this.state.drag){this._drag.cancel();return}if(this.state.armed){this.actions.arm(null);return}if(this.state.dialog){this.actions.dialog(null);return}this.state.review&&!this.state.applying&&this.actions.review(!1)}};this._flip=null;this._narrowQuery=typeof matchMedia=="function"?matchMedia("(max-width: 599px)"):null;this._onWidth=()=>this.requestUpdate();this._route=e=>{let i=ge(e.unique_id,this.state.pending);return i?this.i18n.t("panel.assign.pending.route",{from:this._groupName(e.profile??null),to:this._groupName(i.to)}):""};this._onGrab=(e,i)=>{this._drag.press(e.unique_id,i)};this._onRowPress=(e,i)=>{this._narrow&&!this._locked&&this._drag.arm(e.unique_id,i)};this.i18n=new b,this.state=U({view:"overview",params:{},path:"/"}),this.actions={},this._drag=new Ae({root:()=>this.renderRoot,blocked:()=>this._locked,narrow:()=>this._narrow,onArm:e=>{let i=this._cover(e);i&&this.actions.arm(i)},onStart:e=>{let i=this._cover(e);i&&(this._drag.ghost(i.name,this.renderRoot),this.actions.drag(i))},onOver:e=>this.actions.over(e),onEnd:e=>this._endDrag(e)})}static{this.properties={i18n:{attribute:!1},state:{attribute:!1},actions:{attribute:!1}}}static{this.styles=[E,P,S,D,Ie,hi,gi,Se,mi,yi,u`:host{display:block;background:transparent}.intro{margin:8px 0 4px;max-width:72ch}.intro p{margin:0 0 8px;line-height:1.55}.counts{margin:0 0 16px;color:var(--myhome-text-soft)}.controls{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin:0 0 16px}.controls .search{flex:1 1 220px;max-width:340px}.controls .spacer{flex:1 1 auto}.select-wrap{position:relative;display:inline-flex}.select-wrap:after{content:"";position:absolute;right:13px;top:50%;width:7px;height:7px;border-right:1.6px solid var(--myhome-text-soft);border-bottom:1.6px solid var(--myhome-text-soft);border-radius:1px;transform:translateY(-70%) rotate(45deg);pointer-events:none}select.field{appearance:none;padding-right:36px}a.cta{display:inline-flex;align-items:center;text-decoration:none}.welcome{max-width:640px;margin:48px auto;padding:32px}.welcome h2{margin:0 0 12px;font-size:22px;font-weight:500}.welcome p{margin:0 0 8px;line-height:1.55}.welcome .soft{color:var(--myhome-text-soft);margin-bottom:24px}.welcome .after{margin:12px 0 0;font-size:13px;color:var(--myhome-text-soft)}.notice{padding:16px;margin:16px 0;font-size:14px;line-height:1.55}.notice .actions{margin-top:12px}.groups{margin-bottom:96px}.groups.armed{margin-bottom:120px}.drag-ghost{position:fixed;left:0;top:0;z-index:80;pointer-events:none;background:var(--myhome-card);color:var(--myhome-text);border:1px solid var(--myhome-primary-ink);border-radius:8px;box-shadow:var(--myhome-shadow);padding:10px 14px;font-size:14px;max-width:260px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}`]}connectedCallback(){super.connectedCallback(),window.addEventListener("keydown",this._onKey),this._narrowQuery?.addEventListener("change",this._onWidth)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("keydown",this._onKey),this._narrowQuery?.removeEventListener("change",this._onWidth),this._drag.stop(),this._trap.release()}updated(e){if(this._locked){let i=this._drag.dragging!==null;this._drag.stop(),i&&this.actions.announce(this.i18n.t("panel.assign.announce.drag_cancelled"))}if(e.has("state")){let i=e.get("state");if(this._manageFocus(i),this._flip){let r=this._flip;this._flip=null,requestAnimationFrame(()=>ci(this.renderRoot,r))}}}_beforeMove(){this._flip=di(this.renderRoot)}_manageFocus(e){let i=this.state.dialog!==null||this.state.review,r=(e?.dialog??null)!==null||(e?.review??!1);if(i&&!r){let o=this._activeElement();this._returnTo=o,this._returnToRow=o?.closest("[data-row]")?.getAttribute("data-row")??null,requestAnimationFrame(()=>{let a=this.renderRoot.querySelector("[data-focus-root]");a&&this._trap.hold(a)});return}if(!i&&r){this._trap.release();let o=this._returnTo,a=this._returnToRow;this._returnTo=null,this._returnToRow=null,requestAnimationFrame(()=>{((a?[...this.renderRoot.querySelectorAll("[data-row]")].find(c=>c.getAttribute("data-row")===a)?.querySelector(".handle"):null)??(o?.isConnected?o:null))?.focus()})}}_activeElement(){let e=this.renderRoot.activeElement;return e instanceof HTMLElement?e:null}get _overview(){return this.state.overview}get _locked(){return this._overview?.measuring!=null||this.state.applying}get _narrow(){return this._narrowQuery?.matches??!1}_cover(e){return this._overview?.covers.find(i=>i.unique_id===e)}get _covers(){let e=this._overview;return e?jt(e,this.state.order):[]}get _rooms(){let e=new Set;for(let i of this._overview?.covers??[])i.area&&e.add(i.area);return Array.from(e).sort((i,r)=>i.localeCompare(r,this.i18n.language))}_matches(e){let i=this.state.search.trim().toLowerCase();return i&&!e.name.toLowerCase().includes(i)?!1:!this.state.room||e.area===this.state.room}_groupName(e){return e===null?this.i18n.t("panel.overview.group.no_profile"):this.i18n.t("panel.overview.group.profile",{profile:e})}_valuesLine(e){return e.missing||!e.values||e.values.opening_time===void 0?this.i18n.t("panel.overview.group.values_unknown"):this.i18n.t("panel.overview.group.values",{travel:e.reference_height===null?"?":this.i18n.number(e.reference_height,0),opening:this.i18n.number(e.values.opening_time,1),closing:this.i18n.number(e.values.closing_time,1),slat:this.i18n.number(e.values.slat_time,1)})}_provenanceLine(e){if(e.source==="yaml")return this.i18n.t("panel.overview.group.from_file");if(!e.measured_on)return this.i18n.t("panel.overview.group.provenance_missing");let i=e.measured_at?this.i18n.date(e.measured_at):"";if(!e.measured_on_name)return this.i18n.t("panel.overview.group.measured_on_gone",{date:i});let r=this.i18n.t("panel.overview.group.measured_on",{cover:e.measured_on_name,date:i}),o=(this._overview?.covers??[]).find(a=>a.unique_id===e.measured_on);if(o&&o.profile!==e.name){let a=o.profile??this.i18n.t("panel.overview.group.no_profile");r+=` · ${this.i18n.t("panel.profile.provenance_now_profile",{profile:a})}`}return r}get _groups(){let e=this._overview;if(!e)return[];let i=this._covers,r=a=>i.filter(p=>ve(p,this.state.pending)===a&&this._matches(p)),o=e.profiles.map((a,p)=>({key:a.name,id:`group-${p}`,title:this.i18n.t("panel.overview.group.profile",{profile:a.name}),values:this._valuesLine(a),provenance:this._provenanceLine(a),warning:a.missing?this.i18n.t("panel.overview.group.missing"):"",covers:r(a.name)}));return o.push({key:null,id:"group-none",title:this.i18n.t("panel.overview.group.no_profile"),values:this.i18n.t("panel.overview.group.no_profile_note"),provenance:"",warning:"",covers:r(null)}),o}_appendAtEnd(e,i){if(this.state.order===null||i===(e.profile??null))return;let r=this._covers;this.actions.setOrder(Gt(r.map(o=>o.unique_id),e.unique_id,r.filter(o=>ve(o,this.state.pending)===i).map(o=>o.unique_id)))}_endDrag(e){let i=this.state,r=i.drag,o=r?.insert??null,a=r?.over??null,p=r?this._cover(r.cover):void 0;if(this.actions.drag(null),!e||!p||a===null){r&&this.actions.announce(this.i18n.t("panel.assign.announce.drag_cancelled"));return}let d=fi(a),c=this._covers.map(_=>_.unique_id),m=tt(c,p.unique_id,o??{beforeId:null,afterId:null});this._beforeMove();let h=ge(p.unique_id,i.pending),v=h?h.to:p.profile??null;if(d!==v){this.actions.setOrder(m),this.actions.assign(p,d);return}if(i.pending.length>0){this.actions.setOrder(m),this.actions.announce(this.i18n.t("panel.assign.announce.reordered"));return}this.actions.reorder(m)}_renderControls(){return s`<div class="controls">
      ...
      <p class="after">${this.i18n.t("panel.firstrun.note")}</p>
        ...
        >
      </section>`;if(e.profiles.length===0)return this._renderFirstRun();let i=this._groups,r=i.reduce((p,d)=>p+d.covers.length,0),o=this.state.search.trim()!==""||this.state.room!=="",a=this.state.drag;return s`
...
>
  ...
  ${e}
    ...
    ${e.mode==="remove"?this._remove(e.answer.cover):l}`:s`<div class="card">
      ...
      </div>`}_head(e){let i=[e.area,e.height===null?this.i18n.t("panel.overview.cover.travel_unknown"):this.i18n.t("panel.overview.cover.travel",{travel:this.i18n.number(e.height,0)}),e.profile?this.i18n.t("panel.overview.cover.follows",{profile:e.profile}):null].filter(o=>!!o),r=e.level==="precise"?this.i18n.t("panel.detail.level_thorough"):e.level==="basic"?this.i18n.t("panel.detail.level_basic"):null;return s`<section class="card">
    ...
    </section>`}_view(e,i){let r=j.map(p=>i.find(d=>d.key===p)).filter(p=>p!==void 0),o=e.has_own.length>0,a=o&&e.level!=="precise";return s`<section class="card">
    ...
    </section>`}_field(e){let i=this.state.detail.form[e.key],r=y(e.key,i),o=e.inherited_value===null?this.i18n.t("panel.detail.edit.empty"):this.i18n.t("panel.detail.edit.inherits",{value:this._number(e.key,e.inherited_value)});return le({label:this._label(e.key),value:i??"",unit:this._unit(e.key),placeholder:o,error:r?this._problem(e.key,r):null,disabled:this.state.applying,onInput:a=>this.actions.field(e.key,a)})}_travel(e){let i=this.state.detail.form.height,r=y("height",i),o=this.state.detail.preview,a=o&&o.problem===null?j.filter(p=>o.values[p]!==void 0&&e.values[p]!==void 0):[];return s`<section class="card">
    ...
    </section>`}_remove(e){let i=this.state.detail.answer?.forget;return s`<section class="card">
      ...
      <div class="danger">
        ...
        ${i?.travel_stays?s`<p class="soft">${this.i18n.t("panel.detail.remove.travel_stays")}</p>`:l}
      </div>
      ...
      </div>`;let i=this.state.profile.mode;return s`${this._head(e)}
    ${this.state.writeError?s`<div class="card refusal" role="alert">
          ${this.i18n.refusal(this.state.writeError.translation_key,this.state.writeError.translation_placeholders??{})}
        </div>`:l}
    ...
    ${i==="delete"?this._delete(e):l}`}_head(e){let r=(this.state.overview?.covers??[]).find(p=>p.unique_id===e.measured_on),o;e.measured_on&&e.measured_on_name&&e.measured_at?(o=this.i18n.t("panel.profile.provenance",{cover:e.measured_on_name,date:this.i18n.date(e.measured_at)}),r&&r.profile!==e.name&&(o+=` · ${r.profile?this.i18n.t("panel.profile.provenance_now_profile",{profile:r.profile}):this.i18n.t("panel.profile.provenance_now_none")}`)):e.measured_on&&e.measured_at?o=this.i18n.t("panel.overview.group.measured_on_gone",{date:this.i18n.date(e.measured_at)}):o=this.i18n.t("panel.profile.provenance_missing");let a=e.editable?this.i18n.t("panel.profile.stored"):this.i18n.t("panel.profile.from_file");return s`<section class="card">
    ...
    </section>`}_view(e){let i=this._followers;return s`${e.missing?l:s`<section class="card">
    ...
    </div>`}_edit(){let e=this._followers,i=this.state.profile.form,r=G.some(a=>y(a,i[a])!==null||k(i[a])),o=e.length===1?this.i18n.t("panel.profile.edit.reach_one"):this.i18n.t("panel.profile.edit.reach_all",{count:e.length});return s`<section class="card">
      ...
      <div class="fields">
        ${G.map(a=>le({label:this._label(a),value:i[a]??"",unit:this._unit(a),error:k(i[a])?this.i18n.t("panel.common.required"):(()=>{let p=y(a,i[a]);return p?this._problem(a,p):null})(),disabled:this.state.applying,onInput:p=>this.actions.field(a,p)}))}
      </div>
    ...
    </section>`}_impact(e,i){let r=L.every(p=>e.has_own.includes(p)),o=r?this.i18n.t("panel.profile.impact.state_measured"):e.has_own.length>0?this.i18n.t("panel.profile.impact.state_adjusted"):this.i18n.t("panel.profile.impact.state_inherited"),a;if(r)a=this.i18n.t("panel.profile.impact.no_change");else if(e.height===null)a=this.i18n.t("panel.profile.impact.no_travel");else if(i)a=this.i18n.t("panel.profile.impact.invalid");else{let p=(this.state.profile.impact??[]).find(d=>d.cover_unique_id===e.unique_id);if(!p||p.problem!==null)a=this.i18n.t("panel.common.loading");else if(a=L.filter(c=>!e.has_own.includes(c)&&e.values[c]!==void 0&&p.values[c]!==void 0).map(c=>`${this.i18n.t(`options.step.calibration_edit.data.${c}`)} ${this._number(c,e.values[c])} → ${this._number(c,p.values[c])}`).join(" · "),e.has_own.length>0){let c=this.i18n.t("panel.profile.impact.kept",{keys:this._ownKeys(e)});a=a?`${a} — ${c}`:c}}return s`<div class="row">
      <span class="what">${e.name}</span>
      <span class="instead">${o}</span>
      <span class="from" style="text-align:left">${a}</span>
    </div>`}_rename(e){let i=this.state.profile.newName,r=i.trim()!==""&&!ln.test(i.trim()),o=r?this.i18n.refusal("invalid_name",{profile:i}):this.state.profile.nameError;return s`<section class="card">
    ...
    </section>`}_delete(e){let i=this._followers;return s`<section class="card">
      <h2 data-heading tabindex="-1">
        ${this.i18n.t("panel.profile.delete.title",{profile:e.name})}
      </h2>
      <div class="danger">
        ${i.length===0?s`<p style="margin:0">${this.i18n.t("panel.profile.followers.none")}</p>`:s`<p style="margin:0">${this.i18n.t("panel.profile.delete.affects")}</p>
              <ul>
                ${i.map(r=>s`<li>${r.name}</li>`)}
              </ul>`}
    ...
    </section>`}};customElements.get("myhome-profile-card")||customElements.define("myhome-profile-card",De);var xi=(n,t)=>n.summary?.note?s`<div class="stub">${n.summary.note}</div>`:l;var $i=(n,t)=>{let e=n.options??[];return e.length===0?l:s`<div class="options" role="group" aria-label=${n.title}>
    ${e.map(i=>s`<button
        class="option"
        type="button"
      ...
      >
        <span class="option-head">
          ...
          ${i.chip?s`<span class="chip neutral">${i.chip}</span>`:l}
        </span>
        ${i.meta?s`<span class="option-meta">${i.meta}</span>`:l}
      </button>`)}
  </div>`};var ki=(n,t)=>{let e=n.progress;if(!e)return s`<div class="stub">${t.i18n.t("panel.screen.not_in_this_version")}</div>`;let i=Math.max(0,Math.min(1,e.fraction))*100;return s`<div class="progress-card">
  ...
  </div>`};var Ri=(n,t)=>{let e=n.press;if(!e)return s`<div class="stub">${t.i18n.t("panel.screen.not_in_this_version")}</div>`;let i=e.state==="moving";return s`
  ...
  `};var Ei=(n,t)=>{let e=n.options??[];if(e.length===0&&!n.field)return s`<div class="stub">${t.i18n.t("panel.screen.not_in_this_version")}</div>`;let i=n.field;return s`
    ...
    ${i?s`<div class="reading" style="margin-top:12px">
          ...
          <div class="row">
            ...
            />
            ${i.unit?s`<span class="unit">${i.unit}</span>`:l}
          </div>
          ${i.error?s`<p class="error" id="gap-error" role="alert">${i.error}</p>`:l}
        </div>`:l}
  `};var pn=n=>{let t=[n.hint?"reading-hint":"",n.error?"reading-error":""].filter(e=>e!=="");return t.length>0?t.join(" "):void 0},Pi=(n,t)=>{let e=n.field;return e?s`<div class="reading ${e.big===!1?"":"big"}">
  ...
  </div>`:s`<div class="stub">${t.i18n.t("panel.screen.not_in_this_version")}</div>`};var Si=(n,t)=>{let e=n.summary;return e?s`
    <div class="summary">
      ${e.rows.map(i=>s`<div class="summary-row">
          <span class="label">${i.label}</span>
          ${i.before?s`<span class="before" aria-label=${t.i18n.t("panel.review.before")}
                >${i.before}</span
              >`:l}
  ...
  `:s`<div class="stub">${t.i18n.t("panel.screen.not_in_this_version")}</div>`};var Ti=(n,t)=>n.summary?.rows?.length?s`<div class="summary">
    ...
    </div>`}render(){let e=this.model;if(!e)return l;let i={i18n:this.i18n,fire:this._fire},r=e.model==="pos",o=e.image?Tt(e.image):null;return s`<div class="screen">
    ...
    >
      ←
    </button>`}_renderPlaceholder(e,i){let r={id:"panel.common.not_yet",model:"lettura",title:e,body:i,secondary:[{label:this._i18n.t("panel.common.action.back"),action:"back",kind:"text"}]};return s`<myhome-screen
      .model=${r}
      .i18n=${this._i18n}
      @myhome-screen-action=${o=>{o.detail?.action==="back"&&this._navigate("/")}}
    ></myhome-screen>`}get _locked(){let e=this._store.state;return e.overview?.measuring!=null||e.applying}async _confirm(){let e=this._store.state,i=e.entryId;if(this._locked||!i||e.pending.length===0)return;if(e.pending.filter(p=>{let d=e.overview?.covers.find(c=>c.unique_id===p.cover);return d!==void 0&&Vt(d,p)&&ie(e.heights[p.cover])!==null}).length>0){this._store.set({heightsForced:!0}),this._store.announce(this._i18n.t("panel.assign.announce.missing_travel"));return}let o=it(e.pending,e.heights),a=e.order??void 0;this._store.announce(this._i18n.t("panel.assign.announce.applying")),await this._write(()=>Lt(this.hass.connection,i,o,a),p=>{this._store.set({...be}),this._store.setOverview(p.overview),this._snack(p.applied===1?this._i18n.t("panel.toast.assigned_one"):this._i18n.t("panel.toast.assigned",{count:p.applied}),p.undo_token)})}async _undo(){let e=this._store.state,i=e.snack?.undoToken;if(!i||!e.entryId)return;let r=e.entryId;this._clearSnack(),await this._write(()=>Ut(this.hass.connection,r,i),o=>{this._store.setOverview(o.overview),this._snack(this._i18n.t("panel.toast.undone"),null)})}async _write(e,i,r){this._store.set({applying:!0,writeError:null});try{let o=await e();this._store.set({applying:!1}),i(o)}catch(o){let a=I(o);this._store.set({applying:!1,writeError:a});let p=this._i18n.refusal(a.translation_key,a.translation_placeholders??{});this._store.announce(p),r?.(p)}}_snack(e,i,r=!0){this._clearSnack(),this._store.set({snack:{message:e,undoToken:i}}),r&&this._store.announce(e),this._snackTimer=setTimeout(()=>{this._snackTimer=null,this._store.set({snack:null})},cn)}_clearSnack(){this._snackTimer&&(clearTimeout(this._snackTimer),this._snackTimer=null),this._store.set({snack:null})}_schedulePreview(){this._store.state.review&&(this._previewTimer&&clearTimeout(this._previewTimer),this._previewTimer=setTimeout(()=>{this._previewTimer=null,this._refreshPreview()},at))}async _refreshPreview(){let e=this._store.state;if(!e.entryId||e.pending.length===0){this._store.set({preview:null});return}let i=++this._previewSeq;this._store.set({previewing:!0});try{let r=await me(this.hass.connection,e.entryId,it(e.pending,e.heights));i===this._previewSeq&&this._store.set({preview:r.items,previewing:!1})}catch(r){i===this._previewSeq&&this._store.set({previewing:!1}),te(r)||console.warn("MyHOME panel: the preview could not be read",I(r))}}get _detailCover(){let e=this._store.state,i=e.detail.for;return e.overview?.covers.find(r=>r.unique_id===i)??null}async _loadDetail(){let e=this._store.state,i=e.detail.for;if(!(!this._started||!this.hass||!i||!e.entryId))try{let r=await It(this.hass.connection,e.entryId,i);if(this._store.state.detail.for!==i)return;this._store.set({detail:{...this._store.state.detail,answer:r,loading:!1,error:null}})}catch(r){if(this._store.state.detail.for!==i)return;this._store.set({detail:{...this._store.state.detail,answer:null,loading:!1,error:I(r)}})}}_detailForm(){let e=this._store.state.detail.answer,i={};for(let r of j){let o=e?.keys.find(a=>a.key===r);i[r]=o&&o.own?this._i18n.number(o.value,$[r]??1):""}return i}async _saveValues(){let e=this._store.state,i=e.detail.for;if(this._locked||!i||!e.entryId)return;let r={};for(let d of j){let c=e.detail.form[d];if(y(d,c)!==null)return;r[d]=k(c)?null:M(c??"")}let o=Object.values(r).every(d=>d===null),a=this._detailCover?.name??"",p=e.entryId;await this._write(()=>Dt(this.hass.connection,p,i,r),d=>{this._store.set({detail:{...this._store.state.detail,mode:"view",form:{}}}),this._store.setOverview(d.overview),this._loadDetail(),this._snack(o?this._i18n.t("panel.toast.measure_removed",{cover:a,destination:this._destinationOf(d.overview,i)}):this._i18n.t("panel.toast.values_saved",{cover:a}),d.undo_token)})}async _saveTravel(){let e=this._store.state,i=e.detail.for,r=e.detail.form.height;if(this._locked||!i||!e.entryId||k(r)||y("height",r))return;let o=M(r??""),a=this._detailCover?.name??"",p=e.entryId;await this._write(()=>zt(this.hass.connection,p,i,o),d=>{this._store.set({detail:{...this._store.state.detail,mode:"view",form:{},preview:null}}),this._store.setOverview(d.overview),this._loadDetail(),this._snack(this._i18n.t("panel.toast.travel_saved",{cover:a}),d.undo_token)})}async _removeMeasure(){let e=this._store.state,i=e.detail.for;if(this._locked||!i||!e.entryId)return;let r=this._detailCover?.name??"",o=e.entryId;await this._write(()=>Ht(this.hass.connection,o,i),a=>{this._store.set({detail:{...this._store.state.detail,mode:"view"}}),this._store.setOverview(a.overview),this._loadDetail(),this._snack(this._i18n.t("panel.toast.measure_removed",{cover:r,destination:a.falls_back_to==="profile"?this._i18n.t("panel.detail.destination.profile",{profile:a.profile??""}):a.falls_back_to==="file"?this._i18n.t("panel.detail.destination.file"):this._i18n.t("panel.detail.destination.defaults")}),a.undo_token)})}_destinationOf(e,i){let r=e.covers.find(o=>o.unique_id===i);return r?.origin==="inherited"||r?.origin==="adjusted"?this._i18n.t("panel.detail.destination.profile",{profile:r.profile??""}):r?.origin==="from_the_file"?this._i18n.t("panel.detail.destination.file"):this._i18n.t("panel.detail.destination.defaults")}_scheduleTravelPreview(){this._travelTimer&&clearTimeout(this._travelTimer),this._travelTimer=setTimeout(()=>{this._travelTimer=null,this._refreshTravelPreview()},at)}async _refreshTravelPreview(){let e=this._store.state,i=this._detailCover,r=e.detail.form.height;if(!e.entryId||!i||k(r)||y("height",r)!==null){this._store.set({detail:{...this._store.state.detail,preview:null}});return}let o=++this._previewSeq;this._store.set({detail:{...this._store.state.detail,previewing:!0}});try{let a=await me(this.hass.connection,e.entryId,[{...nt(i),height:M(r??"")}]);if(o!==this._previewSeq)return;this._store.set({detail:{...this._store.state.detail,preview:a.items[0]??null,previewing:!1}})}catch(a){o===this._previewSeq&&this._store.set({detail:{...this._store.state.detail,previewing:!1}}),te(a)||console.warn("MyHOME panel: the preview could not be read",I(a))}}_openFlow(e){Xt({source:e,entryId:this._store.state.entryId,onLeaving:()=>this._store.announce(this._i18n.t("panel.common.opens_configure"))})}_title(){let e=this._store.state,i=e.route;if(i.view==="cover"){let r=this._detailCover;return r?this._i18n.t("panel.detail.named",{cover:r.name}):this._i18n.t("panel.detail.title")}return i.view==="profile"?e.overview?.profiles.some(o=>o.name===i.params.name)?this._i18n.t("panel.profile.name",{profile:i.params.name}):this._i18n.t("panel.profile.title"):this._i18n.t("panel.overview.title")}get _profileRow(){let e=this._store.state;return e.overview?.profiles.find(i=>i.name===e.profile.for)??null}_followersOf(e){let i=this._store.state.overview?.covers??[];if(!e)return[];let r=new Set([...e.followers,...e.followers_from_file]);return i.filter(o=>r.has(o.unique_id))}_profileForm(e){let i={};for(let r of G){let o=r==="reference_height"?e?.reference_height:e?.values[r];i[r]=o==null?"":this._i18n.number(o,$[r]??0)}return i}_typedProfile(){let e=this._store.state.profile.form,i={};for(let r of G){if(k(e[r])||y(r,e[r])!==null)return null;i[r]=M(e[r]??"")}return i}async _saveProfile(){let e=this._store.state,i=e.profile.for,r=this._typedProfile();if(this._locked||!i||!e.entryId||!r)return;let{reference_height:o,...a}=r,p=e.entryId;await this._write(()=>Nt(this.hass.connection,p,i,a,o),d=>{this._store.set({profile:{...this._store.state.profile,mode:"view",impact:null}}),this._store.setOverview(d.overview),this._snack(d.affected.length===1?this._i18n.t("panel.toast.profile_saved_one",{profile:i}):this._i18n.t("panel.toast.profile_saved",{profile:i,count:d.affected.length}),d.undo_token)})}async _renameProfile(){let e=this._store.state,i=e.profile.for,r=e.profile.newName.trim();if(this._locked||!i||!e.entryId||!r||r===i)return;let o=e.entryId;await this._write(()=>qt(this.hass.connection,o,i,r),a=>{this._store.setOverview(a.overview),this._navigate(`/profile/${encodeURIComponent(r)}`),this._snack(this._i18n.t("panel.toast.profile_renamed",{profile:r}),a.undo_token)},a=>this._store.set({profile:{...this._store.state.profile,nameError:a},writeError:null}))}async _deleteProfile(){let e=this._store.state,i=e.profile.for;if(this._locked||!i||!e.entryId)return;let r=e.entryId;await this._write(()=>Ft(this.hass.connection,r,i),o=>{this._store.setOverview(o.overview),this._navigate("/"),this._snack(this._i18n.t("panel.toast.profile_deleted",{profile:i}),o.undo_token)})}_scheduleImpact(){this._impactTimer&&clearTimeout(this._impactTimer),this._impactTimer=setTimeout(()=>{this._impactTimer=null,this._refreshImpact()},at)}async _refreshImpact(){let e=this._store.state,i=this._typedProfile(),r=e.profile.for,o=this._followersOf(this._profileRow);if(!e.entryId||!r||!i||o.length===0){this._store.set({profile:{...this._store.state.profile,impact:null}});return}let a=++this._previewSeq;this._store.set({profile:{...this._store.state.profile,impacting:!0}});try{let p=await me(this.hass.connection,e.entryId,o.map(d=>nt(d)),{[r]:i});if(a!==this._previewSeq)return;this._store.set({profile:{...this._store.state.profile,impact:p.items,impacting:!1}})}catch(p){a===this._previewSeq&&this._store.set({profile:{...this._store.state.profile,impacting:!1}}),te(p)||console.warn("MyHOME panel: the impact preview could not be read",I(p))}}async _retry(){await this._refresh(),this._store.state.connection!=="live"&&await this._listen()}_renderView(){let e=this._store.state;if(e.status==="loading")return e.route.view==="cover"||e.route.view==="profile"?Pe(this._i18n):si(this._i18n);if(e.status==="error"||!e.overview){let r=e.error;return s`<div class="card problem" role="alert">
        <div>
      ...
      </div>`}let i=e.route;return i.view==="cover"?s`<myhome-cover-detail
        .i18n=${this._i18n}
        .state=${e}
        .actions=${this._detailActions}
      ></myhome-cover-detail>`:i.view==="profile"?s`<myhome-profile-card
        .i18n=${this._i18n}
        .state=${e}
        .actions=${this._profileActions}
      ></myhome-profile-card>`:i.view!=="overview"?this._renderPlaceholder(this._i18n.t("panel.overview.title"),this._i18n.t("panel.common.not_yet")):s`<myhome-overview
      .i18n=${this._i18n}
      .state=${e}
      .actions=${this._assignActions}
    ></myhome-overview>`}render(){let e=this._store.state,i=this._title(),r=e.overview?.measuring??null,o=e.route.view!=="overview";return s`
      <!--
    ...
    `}_renderGatewayPicker(){let e=this._store.state,i=e.overview?.entries??[],r=i.find(a=>a.entry_id===e.entryId);if(i.length<=1)return l;let o=this._i18n.t("panel.common.gateway",{gateway:r?.title??""});return s`<select
      class="gateway"
      aria-label=${o}
      .value=${e.entryId??""}
      ?disabled=${e.applying}
    ...
    >
      ${i.map(a=>s`<option value=${a.entry_id} ?selected=${a.entry_id===e.entryId}>
          ${a.title}
        </option>`)}
    </select>`}async _switchGateway(e){if(!e||e===this._store.state.entryId)return;let i=this._unsubscribeWs;this._unsubscribeWs=null,await i?.().catch(()=>{}),this._clearSnack(),this._store.set({...be,entryId:e,detail:re,profile:oe,search:"",room:"",snack:null}),this._navigate("/"),await this._refresh(),await this._listen()}};customElements.get("myhome-calibration-panel")||customElements.define("myhome-calibration-panel",lt);export{lt as MyHomeCalibrationPanel};

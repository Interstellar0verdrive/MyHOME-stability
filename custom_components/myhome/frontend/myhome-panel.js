...
>
  ...
  ${e}
    ...
    </section>`}_field(e){let t=this.state.detail.form[e.key],r=y(e.key,t),o=e.inherited_value===null?this.i18n.t("panel.detail.edit.empty"):this.i18n.t("panel.detail.edit.inherits",{value:this._number(e.key,e.inherited_value)});return le({label:this._label(e.key),value:t??"",unit:this._unit(e.key),placeholder:o,error:r?this._problem(e.key,r):null,disabled:this.state.applying,onInput:a=>this.actions.field(e.key,a)})}_travel(e){let t=this.state.detail.form.height,r=y("height",t),o=this.state.detail.preview,a=o&&o.problem===null?j.filter(p=>o.values[p]!==void 0&&e.values[p]!==void 0):[];return s`<section class="card">
    ...
    ${t==="delete"?this._delete(e):l}`}_head(e){let r=(this.state.overview?.covers??[]).find(p=>p.unique_id===e.measured_on),o;e.measured_on&&e.measured_on_name&&e.measured_at?(o=this.i18n.t("panel.profile.provenance",{cover:e.measured_on_name,date:this.i18n.date(e.measured_at)}),r&&r.profile!==e.name&&(o+=` · ${r.profile?this.i18n.t("panel.profile.provenance_now_profile",{profile:r.profile}):this.i18n.t("panel.profile.provenance_now_none")}`)):e.measured_on&&e.measured_at?o=this.i18n.t("panel.overview.group.measured_on_gone",{date:this.i18n.date(e.measured_at)}):o=this.i18n.t("panel.profile.provenance_missing");let a=e.editable?this.i18n.t("panel.profile.stored"):this.i18n.t("panel.profile.from_file");return s`<section class="card">
    ...
    </div>`}_edit(){let e=this._followers,t=this.state.profile.form,r=G.some(a=>y(a,t[a])!==null||k(t[a])),o=e.length===1?this.i18n.t("panel.profile.edit.reach_one"):this.i18n.t("panel.profile.edit.reach_all",{count:e.length});return s`<section class="card">
      ...
      <div class="fields">
        ${G.map(a=>le({label:this._label(a),value:t[a]??"",unit:this._unit(a),error:k(t[a])?this.i18n.t("panel.common.required"):(()=>{let p=y(a,t[a]);return p?this._problem(a,p):null})(),disabled:this.state.applying,onInput:p=>this.actions.field(a,p)}))}
      </div>
  ...
  `:s`<div class="stub">${i.i18n.t("panel.screen.not_in_this_version")}</div>`};var Ti=(n,i)=>n.summary?.rows?.length?s`<div class="summary">
    ...
    </select>`}async _switchGateway(e){if(!e||e===this._store.state.entryId)return;let t=this._unsubscribeWs;this._unsubscribeWs=null,await t?.().catch(()=>{}),this._clearSnack(),this._store.set({...be,entryId:e,detail:re,profile:oe,search:"",room:"",snack:null}),this._navigate("/"),await this._refresh(),await this._listen()}};customElements.get("myhome-calibration-panel")||customElements.define("myhome-calibration-panel",lt);export{lt as MyHomeCalibrationPanel};

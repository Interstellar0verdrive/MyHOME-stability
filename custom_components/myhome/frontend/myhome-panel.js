...
`);if(s.every(Tn)){e.push(a`<ul>
  ...
  ></div>`,_t=()=>a`<div class="sk-row">${v("62%")}${v("40%","10px")}${v("28%","18px")}</div>`,Hi=()=>a`<div class="sk-group">
    ...
    ${n.locked?a`<span class="locked"
        ...
        >`:l}
  </div>`},He=n=>a`<div class="strip dark applying" role="status">
...
>
  ...
  `:a`<div class="stub">${t.i18n.t("panel.screen.not_in_this_version")}</div>`};var dn=(n,t)=>n.summary?.rows?.length?a`<div class="summary">
    ...
    </select>`}async _switchGateway(e){if(!e||e===this._store.state.entryId)return;let i=this._unsubscribeWs;this._unsubscribeWs=null,await i?.().catch(()=>{}),this._clearSnack();let r=this._sessionClient;this._sessionClient=null,r?.dispose(),this._store.set({...Ce,entryId:e,session:null,sessionError:null,wizardIntent:null,detail:he,profile:ue,search:"",room:"",snack:null}),this._navigate("/"),await this._refresh(),await this._listen()}};customElements.get("myhome-calibration-panel")||customElements.define("myhome-calibration-panel",bt);export{bt as MyHomeCalibrationPanel};

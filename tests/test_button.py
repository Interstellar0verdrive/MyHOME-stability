...
async def test_button_names_and_press(hass: HomeAssistant, tmp_path) -> None:
    ...
    async with setup_myhome(hass, tmp_path, LOCK_YAML) as (_entry, commands):
        lock = hass.states.get("button.light_test_lock")
        unlock = hass.states.get("button.light_test_unlock")
        assert lock.attributes["friendly_name"] == "Light Test Lock"
        assert unlock.attributes["friendly_name"] == "Light Test Unlock"

        await hass.services.async_call(BUTTON, "press", {ATTR_ENTITY_ID: "button.light_test_lock"}, blocking=True)
        await hass.services.async_call(BUTTON, "press", {ATTR_ENTITY_ID: "button.light_test_unlock"}, blocking=True)

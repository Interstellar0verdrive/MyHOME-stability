...
4. Lighting / automation / dry contact / aux / heating events → a WHO 1 command
   translation (`*1*1000#WHAT*WHERE##`) is republished as
   `myhome_light_pushbutton_event` and consumed there; a translation of any other
   type is dropped at DEBUG. Otherwise: handle general/area/group scope by firing
   the matching bus event and re-requesting the affected states, handle dimmer
   preset levels by asking the light for its real brightness, and deliver
   everything else to the entities.
